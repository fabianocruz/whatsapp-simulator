...
export const nearNextTier: TipRule = (ctx) => {
  ...
  for (const category of ['utility', 'authentication'] as const) {
    ...
    return tip({
      ...
      triggeredBy: [],
      currency: ctx.currency,
      titlePt: `Faltam ${next.messagesAway} mensagens para o tier -${next.nextTier.discountPct}%`,
      titleEn: `${next.messagesAway} messages from the -${next.nextTier.discountPct}% tier`,
