...
const BUILDERS: Record<ReasonCode, Builder> = {
  FREE_INBOUND: () => ({
    pt: 'Mensagem do usuário nunca é cobrada, e abre ou renova a janela de atendimento de 24h.',
    en: 'User messages are never billed, and they open or renew the 24h customer service window.',
  }),
  NOT_BILLABLE_NOT_DELIVERED: () => ({
    pt: 'Sem cobrança: a cobrança só acontece quando a mensagem é entregue.',
    en: 'Not billed: charging only happens once the message is delivered.',
  }),
  NOT_BILLABLE_FAILED: () => ({
    pt: 'Sem cobrança: mensagem com falha de entrega não gera cobrança (mas derruba o quality rating).',
    en: 'Not billed: a failed message is not charged (but it does hurt your quality rating).',
  }),
  FREE_IN_FEP: (ctx) => ({
    pt: `Grátis: dentro da janela de free entry point de 72h, aberta por anúncio Click-to-WhatsApp ou CTA de Página. Vale até ${shortTime(ctx.fepActiveUntil, 'pt-BR')} (UTC).`,
  ...
  BILLABLE_MARKETING_TEMPLATE: (ctx) => ({
    pt: `Cobrado: template de marketing é cobrado em toda entrega, dentro ou fora da janela. Rate ${money(ctx, 'pt-BR')}.`,
    en: `Billed: a marketing template is charged on every delivery, in or out of the window. Rate ${money(ctx, 'en-US')}.`,
  }),
  FREE_IN_CSW: (ctx) => ({
    pt: `Grátis: template utility dentro da janela de atendimento de 24h, aberta até ${shortTime(ctx.cswOpenUntil, 'pt-BR')} (UTC). Essa gratuidade acaba em 30/09/2026.`,
  ...
  BILLABLE_AUTHENTICATION_TEMPLATE: (ctx) => ({
    pt: `Cobrado: template de authentication é sempre cobrado na entrega. Rate ${money(ctx, 'pt-BR')}${ctx.discountPct ? ` (tier -${ctx.discountPct}%)` : ''}.`,
  ...
  FREE_SERVICE: () => ({
    pt: 'Grátis: resposta non-template dentro da janela de atendimento. A partir de 01/10/2026 mensagens de service passam a ser cobradas.',
  ...
  FREE_SERVICE_ALLOWANCE: (ctx) => ({
    pt: `Grátis pela franquia: ${ctx.allowanceTotal ?? 1000} mensagens de service por mês por número. Restam ${ctx.allowanceRemaining ?? 0} neste mês.`,
  ...
  BILLABLE_SERVICE: (ctx) =>
    ctx.allowanceEvaluated
      ? {
          pt: `Cobrado: mensagem de service ao rate de utility/authentication do mercado, ${money(ctx, 'pt-BR')}. A franquia de ${ctx.allowanceTotal ?? 1000} mensagens/mês deste número já foi consumida.`,
      ...
      : {
          pt: `Cobrado: mensagem de service ao rate de utility/authentication do mercado, ${money(ctx, 'pt-BR')}. Este é o rate de lista: a franquia de ${ctx.allowanceTotal ?? 1000} mensagens/mês por número é aplicada na projeção mensal, não no custo por conversa.`,
  ...
  INVALID_NON_TEMPLATE_OUTSIDE_CSW: () => ({
    pt: 'Cenário inválido: fora da janela de 24h só é possível enviar template. A Meta rejeitaria este envio, então nada foi cobrado.',
...
export const REASON_LABELS: Record<ReasonCode, LocalizedText> = {
  FREE_INBOUND: { pt: 'grátis · recebida', en: 'free · inbound' },
  NOT_BILLABLE_NOT_DELIVERED: { pt: 'não entregue', en: 'not delivered' },
  NOT_BILLABLE_FAILED: { pt: 'falhou · não cobra', en: 'failed · not billed' },
  FREE_IN_FEP: { pt: 'grátis · FEP 72h', en: 'free · 72h FEP' },
  BILLABLE_MARKETING_TEMPLATE: { pt: 'marketing', en: 'marketing' },
  FREE_IN_CSW: { pt: 'grátis · dentro da CSW', en: 'free · inside CSW' },
  BILLABLE_UTILITY_TEMPLATE: { pt: 'utility', en: 'utility' },
  BILLABLE_AUTHENTICATION_TEMPLATE: { pt: 'authentication', en: 'authentication' },
  FREE_SERVICE: { pt: 'grátis · service', en: 'free · service' },
  FREE_SERVICE_ALLOWANCE: { pt: 'grátis · franquia', en: 'free · allowance' },
  BILLABLE_SERVICE: { pt: 'service', en: 'service' },
  INVALID_NON_TEMPLATE_OUTSIDE_CSW: { pt: 'cenário inválido', en: 'invalid scenario' },
};
